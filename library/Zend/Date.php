...
class Zend_Date extends Zend_Date_DateObject
{
    ...
    public function __construct($date = null, $part = null, $locale = null)
    {
        if (Zend_Locale::isLocale($date)) {
            ...
            $date = null;
            $part = null;
        } elseif (Zend_Locale::isLocale($part)) {
            $locale = $part;
            $part   = null;
        }
        ...
        for ($i = 0; $i < strlen($format); ++$i) {
            if ($format[$i] == "'") {
                if ($comment == false) {
                    ...
                    ++$j;
                    $output[$j] = "'";
                } elseif (isset($format[$i+1]) and ($format[$i+1] == "'")) {
                    $output[$j] .= "'";
                    ++$i;
                } else {
                    $comment = false;
        ...
        switch ($part) {
            ...
            case self::TIME_FULL:
                try {
                    ...
                    return $this->_assign(
                        $calc,
                        $this->mktime($parsed['hour'], $parsed['minute'], 0, $month, $day, $year, true),
                        $this->mktime($hour, $minute, $second, $month, $day, $year, true),
                        false
                    );
                } catch (Zend_Locale_Exception $e) {
    ...
    public function setLocale($locale = null)
    {
        if ($locale instanceof Zend_Locale) {
            $this->_Locale = $locale;
        } elseif (!$locale = Zend_Locale::isLocale($locale, true)) {
            require_once 'Zend/Date/Exception.php';
